...
export function OnboardingScreen({ onComplete }: { onComplete: (profile: UserPreferences) => void }) {
  const [step, setStep] = useState(0);
  const [nickname, setNickname] = useState("");
  const [gradeLevel, setGradeLevel] = useState<GradeLevel | null>(null);
  const [interests, setInterests] = useState<Category[]>([]);
  const [readingLevel, setReadingLevel] = useState<ReadingLevel>("normal");
  const [explanationLevel, setExplanationLevel] = useState<ExplanationLevel>("easy");
  const [dailyArticleCount, setDailyArticleCount] = useState(3);
  const canContinue = step === 0 ? nickname.trim().length >= 2 : step === 1 ? gradeLevel !== null : step === 2 ? interests.length >= 3 : true;
  const next = () => {
    if (!canContinue) return;
    if (step < 3) setStep(step + 1);
    else onComplete({ nickname: nickname.trim(), gradeLevel: gradeLevel!, interests, readingLevel, explanationLevel, dailyArticleCount });
  };
  ...
  return <main className="min-h-dvh bg-[var(--cream)] px-5 py-6 sm:grid sm:place-items-center">
    <div className="mx-auto w-full max-w-[560px]">
      ...
      <section className="card min-h-[500px] p-6 sm:p-9">
        ...
        {step === 2 && <div className="animate-rise"><span className="mb-5 grid h-16 w-16 place-items-center rounded-[22px] bg-[#efeefe] text-3xl">✨</span><p className="eyebrow">나만의 관심 지도</p><h1 className="mt-2 text-[28px] font-black tracking-[-0.04em]">궁금한 것을 골라보세요</h1><p className="mt-2 text-[15px] text-[var(--muted)]">3개 이상 고르면 관심 뉴스와 새로운 탐험 뉴스를 섞어드려요. <b className="text-[var(--green)]">{interests.length}개 선택</b></p><div className="mt-6 grid grid-cols-3 gap-2.5 sm:grid-cols-5">{categories.map(item => { const active = interests.includes(item.name); return <button key={item.name} onClick={() => toggle(item.name)} className={`relative rounded-2xl border-2 px-2 py-4 text-center transition active:scale-95 ${active ? "border-[var(--green)] bg-[var(--green-soft)] shadow-[0_3px_0_#b9dfc2]" : "border-[var(--line)] bg-white hover:bg-[#f8faf8]"}`}><span className="block text-2xl">{item.emoji}</span><span className={`mt-1.5 block text-sm font-extrabold ${active ? "text-[var(--green-deep)]" : "text-[var(--ink)]"}`}>{item.name}</span>{active && <span className="absolute right-1 top-1"><Selected small/></span>}</button>})}</div></div>}
