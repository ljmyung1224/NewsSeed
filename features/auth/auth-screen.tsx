...
export function AuthScreen({ configured, onContinueAsGuest }: { configured: boolean; onContinueAsGuest?: () => void }) {
  ...
  return <main className="grid min-h-dvh place-items-center bg-[var(--cream)] px-5 py-10">
    <section className="w-full max-w-[460px] animate-rise rounded-[28px] border border-[var(--line)] bg-white p-6 text-center shadow-[0_6px_0_#e2e9e4] sm:p-9">
      ...
      {!configured && <div className="mt-5 rounded-2xl bg-[#f5f7f5] p-4"><p className="text-xs font-bold leading-relaxed text-[var(--muted)]">로컬 개발 환경에는 소셜 로그인이 연결되지 않았어요.</p><button onClick={onContinueAsGuest} className="mt-3 text-sm font-black text-[var(--green)] underline underline-offset-4">개발용으로 체험하기</button></div>}
