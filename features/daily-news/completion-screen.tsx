...
export function CompletionScreen({ xp, streak, earnedXp, earnedLeaves, completedCount, totalCount, onHome }: { xp: number; streak: number; earnedXp: number; earnedLeaves: number; completedCount: number; totalCount: number; onHome: () => void }) {
  const dayComplete = completedCount >= totalCount;
  return <main className="relative grid min-h-dvh place-items-center overflow-hidden bg-[var(--green-deep)] px-5 py-10 text-white">
    ...
    <section className="relative z-10 w-full max-w-[500px] animate-rise text-center"><div className="relative mx-auto h-36 w-36"><div className="absolute inset-0 animate-pulse-ring rounded-full bg-[#a8e778]/15"/><div className="absolute inset-4 grid place-items-center rounded-[40px] bg-[#a8e778] text-6xl shadow-[0_8px_0_#6caf52]">🎉</div><span className="absolute -right-3 top-2 grid h-10 w-10 place-items-center rounded-full bg-white text-[var(--green)] shadow-lg"><CheckIcon size={23}/></span></div>
      <p className="mt-8 text-sm font-black tracking-[0.14em] text-[#a9dabb]">{dayComplete ? "TODAY COMPLETE" : "ARTICLE COMPLETE"}</p><h1 className="type-display mt-2 text-[38px] sm:text-5xl">{dayComplete ? "오늘의 신문 완료!" : "기사 하나 완료!"}</h1><p className="mx-auto mt-4 max-w-sm text-[16px] font-medium leading-relaxed text-white/70">{dayComplete ? <>오늘 읽은 뉴스가 새로운 생각 씨앗이 되었어요.<br/>내일도 한 뼘 더 자라 볼까요?</> : <>경험치를 받았어요.<br/>홈에서 다음 씨앗을 골라볼까요?</>}</p>
      ...
      <div className="mx-auto mt-4 max-w-sm rounded-2xl border border-white/10 bg-white/5 p-3 text-sm font-bold text-white/70">지금까지 모은 에너지 <b className="ml-1 text-white">{xp} XP</b>{earnedLeaves > 0 && <span className="ml-3 text-[#b8e897]">+{earnedLeaves} 🍃</span>}</div>
