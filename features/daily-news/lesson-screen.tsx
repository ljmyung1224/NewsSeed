...
export function LessonScreen({ article, index, total, onBack, onComplete }: { article: Article; index: number; total: number; onBack: () => void; onComplete: () => void }) {
  ...
  const question = article.kidContent.quiz[quizIndex];
  const correct = selected === question.answer;
  const isLastQuiz = quizIndex === article.kidContent.quiz.length - 1;

  const submit = () => {
    if (selected === null) return;
    if (!checked) return setChecked(true);
    if (!correct) { setChecked(false); setSelected(null); return; }
    if (isLastQuiz) return onComplete();
  ...
  const lessonProgress = phase === "read" ? 0.25 : 0.55 + ((quizIndex + 1) / article.kidContent.quiz.length) * 0.4;
  ...
  return <main className="min-h-dvh bg-[var(--cream)] pb-28">
    ...
    <article className="mx-auto max-w-[760px] px-5 pt-7 sm:pt-10">
      {phase === "read" ? <div className="animate-rise">
        ...
        <h1 className="text-[30px] font-black leading-[1.28] tracking-[-0.045em] sm:text-[40px]">{article.kidContent.title}</h1>
        <section className="mt-9"><p className="eyebrow">차근차근 읽어요</p><h2 className="mt-1 text-xl font-black sm:text-2xl">쉬운 설명</h2><div className="mt-5 space-y-6 text-[17px] font-medium leading-[1.95] text-[#344139] sm:text-[18px]">{article.kidContent.easyExplanation.map((paragraph, paragraphIndex) => <p key={paragraphIndex}>{renderVocabulary(paragraph, article.kidContent.vocabulary, setVocab)}</p>)}</div></section>
        <section className="mt-10 rounded-[24px] bg-[#eef5ff] p-5 sm:p-7"><p className="text-sm font-black text-[#4c72ad]">생각을 한 뼘 더</p><h2 className="mt-1 text-xl font-black">왜 중요할까요?</h2><div className="mt-4 space-y-4 text-[16px] font-medium leading-[1.85] text-[#39495d] sm:text-[17px]">{article.kidContent.whyItMatters.map((paragraph, paragraphIndex) => <p key={paragraphIndex}>{renderVocabulary(paragraph, article.kidContent.vocabulary, setVocab)}</p>)}</div></section>
        ...
        <blockquote className="my-7 rounded-[22px] border-l-4 border-[var(--green)] bg-[var(--green-soft)] p-5 text-[17px] font-extrabold leading-relaxed text-[var(--green-deep)] sm:p-6"><span className="mb-2 block text-sm">오늘의 한 줄 🌱</span>{article.kidContent.keyTakeaway}</blockquote>
        ...
        <section className="card mt-7 p-5 sm:p-8"><p className="text-xs font-black text-[var(--green)]">Q{quizIndex + 1} / {article.kidContent.quiz.length}</p><h2 className="mt-2 text-xl font-black leading-relaxed">{question.question}</h2><div className="mt-6 space-y-3">{question.options.map((option, optionIndex) => { const isAnswer = optionIndex === question.answer; const optionState = checked ? isAnswer ? "correct" : selected === optionIndex ? "wrong" : "" : selected === optionIndex ? "selected" : ""; return <button disabled={checked} key={`${question.id}-${optionIndex}`} onClick={() => setSelected(optionIndex)} className={`quiz-option ${optionState}`}><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full border-2 border-current text-xs font-black">{String.fromCharCode(65 + optionIndex)}</span><span className="flex-1">{option}</span>{checked && isAnswer && <CheckIcon size={20}/>}</button>; })}</div>
        ...
        <button disabled={selected === null} onClick={submit} className="btn-primary mt-5 w-full">{checked ? correct ? isLastQuiz ? "다음 기사로" : "다음 퀴즈" : "다시 풀기" : "정답 확인하기"}{checked && correct && <ArrowIcon size={18}/>}</button>
