...
export function MonthlyCalendar({ completedDates }: { completedDates: string[] }) {
  ...
  const monthPrefix = `${yearText}-${monthText}`;
  const daysInMonth = new Date(year, month + 1, 0).getDate();
  const days = Array.from({ length: daysInMonth }, (_, index) => index + 1);
  const leading = new Date(year, month, 1).getDay();
  const mondayLeading = (leading + 6) % 7;
  return <section className="card p-5 sm:p-6">
    <div className="mb-5 flex items-end justify-between"><div><h2 className="type-display type-section">나의 성장 기록</h2><p className="mt-1 text-xs font-extrabold text-[var(--green)]">{month + 1}월의 씨앗</p></div><div className="rounded-full bg-[var(--green-soft)] px-3 py-1.5 text-xs font-extrabold text-[var(--green)]">{completedDates.filter(d => d.startsWith(monthPrefix)).length}일 완료</div></div>
    ...
    <div className="grid grid-cols-7 gap-y-2 text-center">{Array.from({length:mondayLeading}).map((_,i)=><div key={`blank-${i}`}/>) }{days.map(day => {
      const date = `${monthPrefix}-${String(day).padStart(2,"0")}`; const done = completedDates.includes(date); const today = date === TODAY;
